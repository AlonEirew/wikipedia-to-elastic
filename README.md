...
* Used in <a href="https://www.aclweb.org/anthology/2021.naacl-main.198/">WEC: Wikipedia Event Coreference</a> research paper
...
### Extracted Relations Types and Features From Wikipedia
...
### Extracted Relations Types and Features From Wikidata
...
* `conf.json` - basic process configuration
```
    ...
    "relationTypes": ["Category", "Infobox", "Parenthesis", "PartName"] (full list at /src/main/java/wiki/data/relations/RelationType.java)
```
...
### Extending Created Index With Additional Wikidata Properties (Postprocess)
...
* Make sure Elastic process is running and active on your host (if running Elastic locally your IP is <a href="http://localhost:9200/">http://localhost:9200/</a>)
* Make sure `wikidata_conf.json` configuration 
* Run the process from command line:<br/>
`java -cp WikipediaToElastic-1.0.jar wiki.wikidata.WikiDataFeatToFile` <br/>
Process will read the full wikidata dump (100M pages), parse and extract the relations, might take a while to finish.
